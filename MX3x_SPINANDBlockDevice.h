 ...
 *
 *  MX3x_SPINANDBlockDevice block_device(SPINAND_FLASH1_IO0, SPINAND_FLASH1_IO1, SPINAND_FLASH1_IO2, SPINAND_FLASH1_IO3,
 ...
 *  @endcode
 */
class MX3x_SPINANDBlockDevice : public mbed::BlockDevice {
public:
    /** Create MX3x_SPINANDBlockDevice - An SPI NAND Flash Block Device over QSPI bus
     *
     ...
     */
    MX3x_SPINANDBlockDevice(PinName io0,
                       PinName io1,
                       PinName io2,
                       PinName io3,
                       PinName sclk,
    ...
    /** Destruct SPINANDBlockDevie
      */
    ~MX3x_SPINANDBlockDevice() override
    {
        MX3x_SPINANDBlockDevice::deinit();
    }
...
private:
    ...
    // Only one MX3x_SPINANDBlockDevice instance per CS is allowed
    ...
    // Read OTP ONFI parameters from the parameter page.
    // This discovers the flash's size, ECC needs, and other attributes.
    bool _read_otp_onfi();
...
private:
    ...
    // Each QSPI Flash device csel can have only 1 MX3x_SPINANDBlockDevice instance
    ...
    // Number of bits ECC correctability per 512 bytes that the flash chip requires from the host.
    // 0 means the chip has hardware ECC and does not need support from the host.
    uint8_t _ecc_bits;

    uint8_t _ecc_bytes, _ecc_steps, _ecc_layout_pos;
    uint32_t  _ecc_size;
    uint8_t *_ecc_calc;
    uint8_t *_ecc_code;
    uint8_t *_page_buf;
